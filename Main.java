...
public class Main {
   ...
   public static void main(String []args) {
      ...
      p1.addBackpack(new Item("Potion", "Heal"));
      ...
      while (game) {
         ...
         move = scan.nextLine();
         if (move.equals("item")){
            System.out.println("What type of item would you like to use?" + p1.getBackpack());
            move = scan.nextLine();
            p1.item(move);
         }
            ...
            for (int i = 0; i < md.getEnemiesLength()-1; i++) {
               p1.fight(md.getEnemy(i));
               try {
                  Thread.sleep(500);
